...
/*
 *  Defines for Attiny platforms, from Mega Tiny Core. 
...
inline void audioOutput(const AudioOutput f) {
  TCA0.SINGLE.CMP0 = f.l()+AUDIO_BIAS;
#if (AUDIO_CHANNELS > 1)
  TCA0.SINGLE.CMP1 = f.r()+AUDIO_BIAS;
#endif
}
...
static void startAudio() {
  ...
  // Two timers needed :
  // 1) PWM output. TCA0 is used for this one, with its three channels we can have a stereo output.
  // 2) refresh rate. TCB0 is used for this one, in periodic interrupt mode (mode 1)

  // Tunning at 20MHz, the PWM resolution can be more than with the usual Mozzi rate (488). It can go up to 610.
  // Setting of TCA0:
  ...
  // Setting TCB0 for audio rate interrupt
/* List of CPU frequencies offered by mega tiny core :
 *  20
 *  16
 *  12 - tuned
 *  10
 ...
 *  2
 *  1
 *  24 - tuned, overclocked
 *  25 - tuned, overclocked
 *  30 - tuned, overclocked
 *  32 - tuned, overclocked, 2-series only.
 */

  // 1221 gives an audio rate  of 16384Hz @ 20MHz.
  TCB0.CCMP = F_CPU / 16384L;
  TCB0.CTRLA = TCB_ENABLE_bm;
  TCB0.CTRLB = 0 << TCB_CNTMODE_gp;
...
// This is the ISR for smapling rate, which updates the PWM output value.
ISR(TCB0_INT_vect){
  defaultAudioOutput();
  TCB0.INTFLAGS = 1;
}
