...
#define IS_DUE() (defined(__SAM3X8E__)) // Arduino Due
#define IS_STM32() (defined(__arm__) && !IS_TEENSY3() && !IS_DUE())  // STM32 boards (note that only the maple based core is supported at this time. If another cores is to be supported in the future, this define should be split.

#if !(IS_AVR() || IS_TEENSY3() || IS_STM32() || IS_DUE())
